...
import { productFactory } from '../../testing/productFactory'
...
describe('ProductInmemoryRepository unit tests', () => {
  ...
  describe('applySort', () => {
    it('should sort items by createdAt desc and ignore non-sortable fields', () => {
      ...
      const products = [
        productFactory({ name: 'c', createdAt: new Date(2026, 0, 1) }),
        productFactory({ name: 'a', createdAt: new Date(2026, 0, 3) }),
        productFactory({ name: 'b', createdAt: new Date(2026, 0, 2) }),
      ]

      sut['items'].push(...products)
    ...
    it('should sort items by name asc/desc according to whitelist', () => {
      const items = [
        productFactory({ name: 'b', price: 10 }),
        productFactory({ name: 'a', price: 20 }),
        productFactory({ name: 'c', price: 30 }),
      ]
